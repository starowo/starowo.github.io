...
$(() => {
  ...
  const fixResult = fixOnlyThink(expectedTags, currentTags, currentMessage);
...
function fixOnlyThink(expectedTags, currentTags, currentMessage) {
  const result = {
    missingTags: [], // 缺失的标签
    fixedTags: [], // 修复的标签
  ...
  for (const currentTag of currentTags) {
    if (currentTag.tagName === '/think_nya~') {
      return result;
    }
  }
  ...
  if (!currentTags[0].noClose) {
    messageToFix = messageToFix.replace(`<${currentTags[0].tagName}>`, `</think_nya~>\n<${currentTags[0].tagName}>`);
    result.fixedTags.push({
      original: `<${currentTags[0].tagName}>`,
      fixed: `</think_nya~>\n<${currentTags[0].tagName}>`,
      reason: `修正第一个标签：从 "${currentTags[0].tagName}" 到 "${currentTags[0].tagName}"`,
    });
