...
function addMenuSection(sectionId, title, content, css = null) {
  ...
  menuDom.find('#s_preset_settings').append(sectionHtml);
}
...
function initializeMenuSections() {
  // 清空现有内容
  menuDom.find('#s_preset_settings').empty();
  menuDom.find('#s_preset_settings').append($(`<h3><strong>预设增强功能</strong></h3>`));
...
function addRegexBindingSection() {
  const regexMenuItems = `
    ...
    <small>将正则表达式绑定到预设中，开启此预设自动加载对应的正则。</small>
    <hr />
  ...
  menuDom.find('#manage_preset_regexes').on('click', function () {
    ...
    $('#extensions-settings-button').click();
